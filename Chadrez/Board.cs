...
namespace Chess
{
	...
	{
		private int[,] square;//2-D Array para manter as inclusões de cada quadrado
		...
		public Board()
		{
			...
			for(i=0;i<8;i++)
			{
				square[i,1]=6;//Pretas peões
				square[i,6]=12;//Brancas peões
			}
		...
		{
			if( (i + j) % 2 == 0)
				return 2;//A cor traseira é Branca
			else 
				return 1;//A cor traseira é Preta
		}
